...
Err.extend('terminal', {
  message: 'The datastream is terminal and cannot be written to.',
  code: 'ETERM'
});
Err.extend('overflow', {
  message: 'Data to write exceeds max file size.',
  code: 'EOVER'
});
...
module.exports = function(directoryPath, configuration) {
  ...
  if (!store.hasOwnProperty(key))
    store[key] = getFactory(directoryPath, configuration);
  return store[key];
};
...
function getFactory(directoryPath, configuration) {
  ...
  function createNewStream() {
    if (!findingPath) {
      findingPath = dirFiles(directoryPath).then(
        function(fileNames) {
          ...
          while (fileNames.indexOf(fileName) !== -1)
            fileName = rfName.increment(fileName);
          fullPath = path.resolve(directoryPath, fileName);

          // set globals to initial write state
          end = config.interval
            ? rfName.components(fileName).date.getTime() + config.interval
            : null;
          findingPath = false;
          size = 0;
